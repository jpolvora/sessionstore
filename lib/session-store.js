...
const SessionStore = (function () {
    ...
    const sessionData = {
      ...
      destroy: async function (cb) {
        if (!res.headersSent) {
          ...
          res.cookie(cookieOpts.name, this.uid, cookieOpts);
        }
  ...
  SessionStore.prototype.middleware = async function (req, res, next) {
    ...
    if (cookieValue) {
      ...
      if (storedSession) {
        ...
        cache.put(cookieValue, sessionData);
      } else {
        cache.del(cookieValue);
      }
    }
  ...
  SessionStore.prototype.startSession = async function (sessionData, res) {
    ...
    res.cookie(cookieOpts.name, sessionData.uid, cookieOpts);
  ...
  SessionStore.prototype.setSession = async function (sessionData) {
    ...
    try {
      ...
      const uid = await self.store.createOrUpdateSession(sessionData.uid, cloned);
      if (uid) {
        cache.put(uid, sessionData);
