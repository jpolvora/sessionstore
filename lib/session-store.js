...
const SessionStore = (function () {
  ...
  SessionStore.prototype.middleware = async function (req, res) {
    const self = this;
  ...
  SessionStore.prototype.setSession = async function (sessionData) {
    ...
    if (sessionData._isModified || sessionData._isNewSession) {
      ...
      try {
        ...
        delete cloned._key;
        await self.store.createOrUpdateSession(sessionData._key, cloned);
        this.originalSessionData = cloned;
...
module.exports = function (opts) {
  ...
  sessionStore.middleware = async function (req, res, next) {
    console.log('attending request', req.originalUrl);
    await sessionStore.middleware(req, res);
    return next();
  }
  return sessionStore;
}
